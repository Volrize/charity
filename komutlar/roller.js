...
exports.run = async (client, message, args) => {
  ...
  const prefix = 'c!';// sizin prefixiniz
  ...
  if(args[0] && args[0] === 'sırala') {
    ...
    .setThumbnail("https://cdn.discordapp.com/avatars/680511287209492493/45e0aa822edbeea709b0d48b53a41441.webp?size=128")
...
${message.guild.roles.cache.filter(x => x.name !== '@everyone').sort((a, b) => b.position-a.position).map(role => `${role} (**${role.members.size}**)`).join('\n')}`)
    .setFooter(`Üye sayısını sıralamak için ${prefix}roller sırala`)
    .setColor('#86FF0E'));
