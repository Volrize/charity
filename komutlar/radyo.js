...
//const pre = db.fetch(`pre_${message.guild.id}`)

//if(pre == 'kapali') return message.channel.send("<:sagoki:840956405090615316> Bu Komut __Premium Sunucular__ İçin Geçerlidir.")

    if (!message.member.voice.channel) return message.channel.send(`${message.author} Çok akıllısın. Sesli odada olmadan nasıl radyo dinlemeyi düşünüyorsun?`)

    if (args[0] === "durdur") {

        if (message.guild.me.voice.channel === null) return message.channel.send(`${message.author} Radyoyu durdurmak için önce bir radyo açman gerek.`)
    ...
    message.channel.send("Radyolar hazırlanıyor...").then(radio => {
